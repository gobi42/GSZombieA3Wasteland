...
if (isServer) then
{
	_building = nearestObjects [_npc; ["Land_Cargo_HQ_V2_F"], 200];
//	_building = nearestBuilding _npc;
	_npc setVariable ["storeNPC_nearestBuilding", netId _building, True]; //Changed From True to false
}
else
{
	...
	
	{
		if (_x select 0 == _npcName) exitWith
		{
			...
			switch (toUpper typeName _npcPos) do
			{
				case "SCALAR":
				{
					_bPos = _building nearestObjects [_npcPos; ["Land_Cargo_HQ_V2_F"], 200];
